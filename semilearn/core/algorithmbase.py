...
class AlgorithmBase:
    ...
    def set_dataset(self):
        ...
        self.print_fn('labeled dataset: {}'.format(dataset_dict['lb_idx'][:5]))
        np.savetxt("{}/train_labeled_idxs.csv".format(self.args.save_dir), dataset_dict['lb_idx'], delimiter=",")
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            self.print_fn('')
            self.print_fn('Epoch: {epoch}/{epochs:4} Train Start:'.format(
                epoch=epoch+1,
                epochs=self.epochs,)
            )
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False):
        ...
        with torch.no_grad():
            for data in eval_loader:
                ...
                y_pred1, y_pred5 = accuracy(logits, y, topk=(1, 5))
                y_correct1 += y_pred1.item()
                y_correct5 += y_pred5.item()
        ...
        topk1 = y_correct1 / total_num * 1.0
        topk5 = y_correct5 / total_num * 1.0
        ...
        eval_dict = {eval_dest+'/loss': total_loss / total_num, eval_dest+'/top-1-acc': top1, eval_dest+'/top-k1-acc': topk1, eval_dest+'/top-k5-acc': topk5, 
                     eval_dest+'/balanced_acc': balanced_top1, eval_dest+'/precision': precision, eval_dest+'/recall': recall, eval_dest+'/F1': F1}
        if return_logits:
            eval_dict[eval_dest+'/logits'] = y_logits

        self.all_confusion.append(cf_mat)
        np.save("{}/all_confusion.npy".format(self.args.save_dir), np.array(self.all_confusion))
