...
class LoggingHook(Hook):
    ...
    def after_train_step(self, algorithm):
        ...
        if self.every_n_iters(algorithm, algorithm.num_log_iter):
            if not algorithm.distributed or (algorithm.distributed and algorithm.rank % algorithm.ngpus_per_node == 0):
                print_text = f"{algorithm.it + 1} iteration train: USE_EMA: {algorithm.ema_m != 0}, "
                for i, (key, item) in enumerate(algorithm.log_dict.items()):
                    print_text += "{:s}: {:.4f}".format(key, item)
                    if i != len(algorithm.log_dict) - 1:
                        print_text += ", "
        ...
        if self.every_n_iters(algorithm, algorithm.num_eval_iter):
            if not algorithm.distributed or (algorithm.distributed and algorithm.rank % algorithm.ngpus_per_node == 0):
                algorithm.print_fn('Epoch: {epoch}/{epochs:4} Evaluation Start:'.format(
                    epoch=algorithm.epoch+1,
                    epochs=algorithm.epochs,)
                )
                print_text = f"{algorithm.it + 1} iteration eval: "
                for i, (key, item) in enumerate(algorithm.eval_dict.items()):
                    print_text += "{:s}: {:.4f}".format(key, item)
                    if i != len(algorithm.eval_dict) - 1:
                        print_text += ", "
                    else:
                        print_text += " "
                algorithm.print_fn(print_text)
                algorithm.print_fn('Epoch: {epoch}/{epochs:4} Test Start:'.format(
                    epoch=algorithm.epoch+1,
                    epochs=algorithm.epochs,)
                )
                algorithm.print_fn(f"{algorithm.it + 1} iteration test: BEST_EVAL_ACC: {algorithm.best_eval_acc}, at {algorithm.best_epoch + 1} epoch, {algorithm.best_it + 1} iters")
