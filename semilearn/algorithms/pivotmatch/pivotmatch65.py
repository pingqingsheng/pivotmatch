...
@ALGORITHMS.register('pivotmatch65')
class PivotMatch65(AlgorithmBase):
    ...
    def init_params(self):
        ...
        self.sharpen_T = self.args.sharpen_T
        # loss function
    ...
    def get_pivot_code(self, pivots, pivots_label, feats, pseudo_label, idx_conf):
        ...
        queue_code = q_sample[num_conf:]
        return pivot_code.detach(), queue_code.detach(), q_sample.detach(), feats_sample

    def get_contrast_label(self, q_conf, q_queue_conf, pivots_label, pseudo_label, idx_non_conf):
        # clustering code for mini-batch
        q_non_conf = pseudo_label[idx_non_conf].reshape(-1, 1).eq(pivots_label.reshape(1, -1))
        q_batch = torch.cat([q_conf, q_non_conf], dim=0)
        q_batch = q_batch / q_batch.sum(1, keepdim=True)
        # clustering code for queue
        idx_conf_queue = torch.where(self.queue_conf_set > 0)[0]
        q_queue = torch.ones((self.queue_size, self.num_pivots)).to(q_conf.device)
        q_queue[idx_conf_queue] = q_queue_conf
        q_queue = q_queue / q_queue.sum(1, keepdim=True)
        # contrast target
        score_mask = torch.mm(q_batch, q_queue.t())
        row_mask = torch.ones(q_batch.shape[0]).float().to(q_conf.device)
        column_mask = self.queue_conf_set
        selection_mask = torch.outer(row_mask, column_mask)
        contrast_target = score_mask * selection_mask
        contrast_target = contrast_target / contrast_target.sum(1, keepdim=True)
        contrast_target = contrast_target.detach()
        return contrast_target
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s, y_ulb):
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                if self.epoch >= self.epoch_warm_up:
                    ...
                    idx_conf = torch.where(mask_da > 0)[0]
                    idx_non_conf = torch.where(mask_da < 0.5)[0]
                    q_conf, q_queue_conf, q_sample, feats_sample = self.get_pivot_code(pivots, pivots_label, feats_u_w, lbs_u_guess_da, idx_conf)

                # label propagation with pivots 
                if self.epoch >= self.epoch_warm_up:
                    # update labels
                    # cluster_u_w_ot = self.get_clustering_label(pivots, pivots_label, feats_u_w)
                    # cluster_u_w_ot_sharpened = cluster_u_w_ot ** (1 / self.sharpen_T)
                    num_feats = feats_u_w.shape[0]
                    feats_all = torch.cat([feats_u_w, self.queue_feats], 0)
                    sim = torch.exp(torch.mm(feats_all, feats_sample.t()) / self.temperature)
                    weight = sim / sim.sum(1, keepdim=True)
                    q_propagated = torch.mm(weight, q_sample)
                    cluster_u_w = torch.zeros((batch_size_u, self.num_classes)).to(feats_u_w.device)
                    cluster_u_w = cluster_u_w.scatter_add(1, pivots_label.expand((batch_size_u, -1)), q_propagated)
                    cluster_u_w_ot = ot_sinkhorn(-cluster_u_w, epsilon=self.epsilon2, exp=False)[:num_feats]
                    cluster_u_w_ot /= cluster_u_w_ot.sum(1, keepdim=True) 
                    cluster_u_w_ot = cluster_u_w_ot.detach()
                    ...
                    cluster_u_w = torch.rand(probs_u_w_da.shape).to(probs_u_w_da.device)
                ...
                lbs_u_guess_cluster, max_probs_cluster, mask_cluster = self.get_lbs_and_masks(cluster_u_w)
                lbs_u_guess_cluster_ot, max_probs_cluster_ot, mask_cluster_ot = self.get_lbs_and_masks(cluster_u_w_ot)
            
                # target for pivots learning & representation learning 
                if self.epoch >= self.epoch_warm_up:
                    # idx_non_conf = torch.where(mask < 0.5)[0]
                    # get contrast target for representation learning 
                    # contrast_target = self.get_contrast_label(q_conf, q_queue_conf, pivots_label, lbs_u_guess, idx_non_conf)
                    # clustering code for queue
                    idx_conf_queue = torch.where(self.queue_conf_set > 0)[0]
                    q_queue = torch.ones((self.queue_size, self.num_pivots)).to(q_conf.device)
                    q_queue[idx_conf_queue] = q_queue_conf
                    q_queue = q_queue / q_queue.sum(1, keepdim=True)
                    # contrast target
                    q_batch = torch.ones(feats_u_w.shape[0], self.num_pivots).float().to(feats_u_w.device)
                    q_batch[idx_conf] = q_conf
                    q_batch[idx_non_conf] = q_propagated[idx_non_conf]
                    label_mask = lbs_u_guess.reshape(-1, 1).eq(torch.max(self.queue_probs, dim=1)[1].reshape(1, -1))
                    score_mask = torch.mm(q_batch, q_queue.t())
                    row_mask = torch.ones(q_batch.shape[0]).float().to(q_conf.device)
                    column_mask = self.queue_conf_set
                    selection_mask = torch.outer(row_mask, column_mask)
                    contrast_target = score_mask * selection_mask * label_mask
                    contrast_target = contrast_target / contrast_target.sum(1, keepdim=True)
                    contrast_target = contrast_target.detach()
            ...
            if self.epoch >= self.epoch_warm_up:
                ## pivot loss
                logits = self.model.pivots_layer(feats_sample.detach())
                q_sample = torch.max(q_sample, dim=1)[1]
                target = F.one_hot(q_sample, num_classes=self.args.num_pivots)
                logits, target = logits.t(), target.t()
                target = target / (target.sum(1, keepdim=True) + 1e-6)
                loss_p = self.loss_p(logits / self.temperature, target)    
                # set loss as 0 until there are enough confident data for clustering 
                if torch.isnan(loss_p):
                    loss_p = torch.tensor(0, dtype=torch.float).to(q_conf.device)
                ## contrast loss
                queries = feats_u_s
                queue = self.queue_feats.clone().detach()
                logits = torch.einsum('nc,kc->nk', [queries, queue])
        ...
        # calculate pseudo label acc
        right_labels_cluster_ot = (lbs_u_guess_cluster_ot == targets_u).float() * mask_cluster_ot
        pseudo_label_acc_cluster_ot = right_labels_cluster_ot.sum() / max(mask_cluster_ot.sum(), 1.0)
        ...
        log_dict = self.process_log_dict(
            ...
            mask_cluster_ot=mask_cluster_ot.float().mean().item(),
            pseudo_label_acc_cluster_ot=pseudo_label_acc_cluster_ot.item(),
