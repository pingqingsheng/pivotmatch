...
# strong + hard for pivot, pivot + queue contrast representation, pivot propagation
#  
...
@ALGORITHMS.register('pivotmatch42')
class PivotMatch42(AlgorithmBase):
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                if self.epoch >= self.epoch_warm_up:
                    ...
                    pivot_code = self.get_pivot_code(pivots, pivots_label, feats_u_w, lbs_u_guess, idx_conf)
            ...
            if self.epoch >= self.epoch_warm_up:
                ## pivot loss
                loss_p = self.loss_p(affinity_u_s[idx_conf] / self.temperature, torch.max(pivot_code, dim=1)[1])
