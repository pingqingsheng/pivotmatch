...
@ALGORITHMS.register('pivotmatch61')
class PivotMatch61(AlgorithmBase):
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                if self.epoch >= self.epoch_warm_up:
                    ...
                    cluster_u_w_ot_sharpened = cluster_u_w_ot
                    q_u_final = self.alpha * probs_u_w_da + (1 - self.alpha) * cluster_u_w_ot_sharpened
                    q_u_final = q_u_final.detach()
                    # classification & clustering label  
                    p_max, p_label = torch.max(probs_u_w_da, dim=1)
                    c_max, c_label = torch.max(cluster_u_w_ot_sharpened, dim=1)
                    mask_label_consistency = (p_label == c_label).float().detach()  
                else:
                    cluster_u_w_ot_sharpened = torch.rand(probs_u_w_da.shape).to(probs_u_w_da.device)
                ...
                lbs_u_guess_cluster, max_probs_cluster, mask_cluster = self.get_lbs_and_masks(cluster_u_w_ot_sharpened)
            ...
            if self.epoch >= self.epoch_warm_up:
                ## pivot loss
                logits_p = self.model.pivots_layer(feats_sample.detach())
                q_sample = torch.max(q_sample, dim=1)[1]
                pivot_target = F.one_hot(q_sample, num_classes=self.args.num_pivots)
                logits_p, pivot_target = logits_p.t(), pivot_target.t()
                pivot_target = pivot_target / (pivot_target.sum(1, keepdim=True) + 1e-6)
                loss_p = self.loss_p(logits_p / self.temperature, pivot_target)    
                ...
                logits_c = torch.einsum('nc,kc->nk', [queries, queue])
                loss_c = self.loss_c(logits_c / self.temperature, contrast_target)              
            else:
