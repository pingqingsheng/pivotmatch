...
@ALGORITHMS.register('pivotmatch62')
class PivotMatch62(AlgorithmBase):
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                if self.epoch >= self.epoch_warm_up:
                    ...
                    sim = torch.exp(torch.mm(feats_u_w, self.queue_feats.t()) / self.temperature)
                    weight = sim * self.queue_conf_set
                    weight = weight / weight.sum(1, keepdim=True)
                    ...
                    cluster_u_w_ca = torch.rand(probs_u_w_da.shape).to(probs_u_w_da.device)
                    q_u_final = probs_u_w_da.clone().detach()
                ...
                lbs_u_guess_cluster, max_probs_cluster, mask_cluster = self.get_lbs_and_masks(cluster_u_w_ca)
                ...
                if self.epoch >= self.epoch_warm_up:
                    ...
                    q_conf, q_queue_conf, q_sample, feats_sample = self.get_pivot_code(pivots, pivots_label, feats_u_w, lbs_u_guess, idx_conf)
                    ...
                    label_mask =  torch.mm(F.one_hot(l_i, self.num_classes).float(), F.one_hot(l_j, self.num_classes).float().t())
                    score_mask = torch.outer(q_i, q_j)
                    row_mask = q_i.ge(0).float()
                    column_mask = self.queue_conf_set
                    select_matrix = torch.outer(row_mask, column_mask)
                    # contrast target
                    contrast_target = label_mask * score_mask * select_matrix
                    contrast_target = contrast_target / contrast_target.sum(1, keepdim=True)
                    contrast_target.detach()
