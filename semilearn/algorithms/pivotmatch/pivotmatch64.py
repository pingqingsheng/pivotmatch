...
@ALGORITHMS.register('pivotmatch64')
class PivotMatch64(AlgorithmBase):
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                if self.epoch >= self.epoch_warm_up:
                    ...
                    sim = torch.exp(torch.mm(feats_u_w, feats_sample.t()) / self.temperature)
                    ...
                    q_u_final = self.alpha * probs_u_w_da + (1 - self.alpha) * cluster_u_w
                    q_u_final = q_u_final.detach()
                    # classification & clustering label  
                    p_max, p_label = torch.max(probs_u_w_da, dim=1)
                    c_max, c_label = torch.max(cluster_u_w, dim=1)
