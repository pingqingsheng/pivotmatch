...
@ALGORITHMS.register('pivotmatch23')
class PivotMatch23(AlgorithmBase):
    """
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s_0, x_ulb_s_1, y_ulb):
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                # ema feature & logits for all in queue required by ImageNet
                    inputs_ema = torch.cat((x_lb, x_ulb_w)) 
                    outputs_ema = self.model(inputs_ema)
                    logits_ema, feats_ema = outputs_ema['logits'], outputs_ema['feat']

                    _, feats_x_lb_ema = logits_ema[:num_lb], feats_ema[:num_lb]
                    logits_x_ulb_w_ema, feats_x_ulb_w_ema = logits_ema[num_lb:], feats_ema[num_lb:]

                    feats_x_lb = feats_x_lb_ema
                    feats_x_ulb_w = feats_x_ulb_w_ema
                    logits_x_ulb_w = logits_x_ulb_w_ema
            ...
            with torch.no_grad():
                ...
                # label matching
                if self.epoch > 0: 
                    sim = torch.exp(torch.mm(feats_x_ulb_w, self.queue_feats_w.t()) / self.T)
                    sim = sim * self.queue_pivots
                    sim = sim / sim.sum(1, keepdim=True)
                    
                    cluster_x_ulb_w = torch.mm(sim, self.queue_probs)
                    cluster_x_ulb_w = self.call_hook("dist_align", "ClusterAlignHook", probs_x_ulb=cluster_x_ulb_w.detach())

                    q_u = self.alpha * probs_x_ulb_w + (1 - self.alpha) * cluster_x_ulb_w
                    q_u = q_u.detach()

                    # classification & clustering label  
                    p_max, p_label = torch.max(probs_x_ulb_w, dim=1)
                    c_max, c_label = torch.max(cluster_x_ulb_w, dim=1)
                ...
                mask_pivot = max_probs.ge(self.pivot_p_cutoff).float()
