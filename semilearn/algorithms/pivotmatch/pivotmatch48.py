...
@ALGORITHMS.register('pivotmatch48')
class PivotMatch48(AlgorithmBase):
    ...
    def get_pivot_code(self, pivots, pivots_label, feats, pseudo_label, idx_conf):
        ...
        return pivot_code.detach(), q_sample.detach(), feats_sample
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                if self.epoch >= self.epoch_warm_up:
                    ...
                    cluster_u_w_ot_sharpened = cluster_u_w_ot_sharpened / cluster_u_w_ot_sharpened.sum(1, keepdim=True)
            ...
            if self.epoch >= self.epoch_warm_up:
                ...
                if torch.isnan(loss_p):
                    loss_p = torch.tensor(0, dtype=torch.float).to(pivot_code.device)
                ## contrast loss
