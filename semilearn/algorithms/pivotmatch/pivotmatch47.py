...
@ALGORITHMS.register('pivotmatch47')
class PivotMatch47(AlgorithmBase):
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                if self.epoch >= self.epoch_warm_up:
                    ...
                    # cluster_u_w_ot_sharpened = cluster_u_w_ot_sharpened / cluster_u_w_ot_sharpened.sum(dim=-1, keepdim=True)
            ...
            if self.epoch >= self.epoch_warm_up:
                ...
                loss_p = self.loss_p(logits / self.temperature, torch.max(q_sample, dim=1)[1])    
