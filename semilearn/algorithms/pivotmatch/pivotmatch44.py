...
@ALGORITHMS.register('pivotmatch44')
class PivotMatch44(AlgorithmBase):
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s, y_ulb):
        ...
        with self.amp_cm():
            ...
            if self.epoch >= self.epoch_warm_up:
                ## pivot loss
                loss_p = self.loss_p(affinity_u_w[idx_conf] / self.temperature, torch.max(pivot_code, dim=1)[1])
